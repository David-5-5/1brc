 ...
 */
package dev.morling.onebrc.refer;
...
public class Statistics {
    ...
    public static void distribution() throws Exception {
        try (var in = new BufferedReader(new FileReader(MEASUREMENTS_TXT))) {
            ...
            while (true) {
                ...
                // actual nameLen in 1BRC code includes the semicolon, this is deliberately one
                // less,
                // so we get buckets that correspond to the condition nameLen > 8 and nameLen >
                // 16
                var nameLen = line.indexOf(';');
                // use "+= nameLen" here to get byte distribution instead: for a randomly chosen
                // name byte
