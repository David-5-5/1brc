...
public class Blog5 {
    ...
    private static class ChunkProcessor implements Runnable {
        ...
        public void run() {
            processChunk();
            results[myIndex] = Arrays.stream(hashtable).filter(Objects::nonNull).map(StationStats::new)
        ...
        private StatsAcc ensureAcc(long hash, long nameStartOffset, int nameLen,
            ...
            while (true) {
                ...
                if (acc.hash == hash
                        && acc.nameEquals(inputBase, nameStartOffset, nameLen, nameWord0, nameWord1, lastNameWord)) {
                    return acc;
                }
    ...
    static class StatsAcc {
        ...
        boolean nameEquals(long inputBase, long inputNameStart, long inputNameLen, long inputWord0, long inputWord1,
                           long lastInputWord) {
